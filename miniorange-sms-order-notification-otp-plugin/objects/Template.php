...
abstract class Template extends BaseActionHandler implements MoITemplate
{
	...
	public function savePopup()
	{
		...
		$template = stripslashes(sanitize_text_field($_POST[$this->getTemplateEditorId()]));
		$this->validateRequiredFields($template);
	...
	{
		return array_key_exists('popuptype',$_POST) && strcasecmp(sanitize_text_field($_POST['popuptype']),$this->getTemplateKey())==0;
	}
