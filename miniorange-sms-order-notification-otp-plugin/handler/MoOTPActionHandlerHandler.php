...
class MoOTPActionHandlerHandler extends BaseActionHandler
{
    ...
    $queryServer = sanitize_text_field($_SERVER['QUERY_STRING']);
    $current_url    = admin_url().'admin.php?'.$queryServer;
	...
	function _handle_admin_actions()
	{
		...
		switch($option)
		{
			case "mo_customer_validation_settings":
				$this->_save_settings($data);																	 break;
			case "mo_customer_validation_messages":
				$this->_handle_custom_messages_form_submit($data);												 break;
			case "mo_validation_contact_us_query_option":
				$this->_mo_validation_support_query($data);                                                     break;
			case "mo_otp_extra_settings":
				$this->_save_extra_settings($data); 															 break;
            ...
            case "mo_customer_validation_sms_configuration":
                $this->_mo_configure_sms_template($data);														 break;
            case "mo_customer_validation_email_configuration":
                $this->_mo_configure_email_template($data);													 break;
            case "mo_customer_customization_form":
            	$this->_mo_configure_custom_form($data);														 break;
		}
	}
...
function _mo_configure_custom_form($post){
	...
	{
		$this->isValidRequest();
		$gatewayType = sanitize_text_field($_POST['gateway_type']);
