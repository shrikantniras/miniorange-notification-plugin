...
class WooCommerceRegistrationForm extends FormHandler implements IFormHandler
{
    ...
    function woocommerce_site_registration_errors(WP_Error $errors,$username,$password,$email)
    {
        ...
        if($this->_isAjaxForm){
            $this->assertOTPField($errors, sanitize_post($_POST));
            $this->checkIfOTPWasSent($errors);
            return $this->checkIntegrityAndValidateOTP( sanitize_post($_POST),$errors);
        }	else {
    ...
    function handleFormOptions()
    {
        ...
        $this->_redirectToPage = isset( $_POST['page_id']) ? get_the_title(sanitize_text_field($_POST['page_id'])) : 'My Account';
