...
class MoUtility
{
    ...
    {   
        $httpIPServer = sanitize_text_field($_SERVER['HTTP_CLIENT_IP']);
        $httpXServer = sanitize_text_field($_SERVER['HTTP_X_FORWARDED_FOR']);
        $remoteAddrServer = sanitize_text_field($_SERVER['REMOTE_ADDR']);
        if (!empty($httpServer)) {
            $moIP = $httpServer;
        }
        else if (!empty($httpXServer)) {
            $moIP= $httpXServer;
        }
        else {
            $moIP= $remoteAddrServer;
        }
    ...
    {
        $pageURL = 'http';
        $httpsServer = sanitize_text_field($_SERVER["HTTPS"]);
        $portServer = sanitize_text_field($_SERVER["SERVER_PORT"]);
        $request_Uri = sanitize_text_field($_SERVER["REQUEST_URI"]);
        $serverName = sanitize_text_field($_SERVER["SERVER_NAME"]);

        if ((isset($httpsServer)) && ($httpsServer == "on"))
            $pageURL .= "s";

        $pageURL .= "://";

        if ($portServer != "80")
            $pageURL .= $serverName . ":" . $portServer . $request_Uri;

        else
            $pageURL .= $serverName . $request_Uri;
    ...
    public static function sanitize_post($post)
    {
        ...
        foreach ($post as $key => $value) {
            ...
            $sanitizedPost[$sanitizedkey] = sanitize_text_field($value);
        }
